...
const studentDetails=[{"id":1,"first_name":"Darby","last_name":"Headrick","email":"dheadrick0@discovery.com","state":"AZ","science":99,"maths":99,"english":99,"social":99},
...
{"id":100,"first_name":"Anni","last_name":"Harley","email":"aheitz2r@seesaa.net","state":"TX","science":0,"maths":90,"english":0,"social":0}]

findBestStudent(studentDetails);
findLastStudent(studentDetails);
findBestState(studentDetails);
getPaginationResult(studentDetails, 4, 5);


//the student with best average
function findBestStudent(studentDetails){
    let maxAverage=0;
    let firstStudent="";
   
    studentDetails.forEach((mark)=>{

       let average=(mark["science"]+mark["maths"]+mark["english"]+mark["social"])/4;
        if(average>maxAverage){
         maxAverage=average;
         firstStudent=mark["first_name"]+" "+mark["last_name"];
        }
    })
    console.log(firstStudent,maxAverage);
}


//the student with least average
function findLastStudent(studentDetails){
let leastAverage=100;
lastStudent="";

studentDetails.forEach((mark)=>{
    let average=0;
   average+=mark["science"]>0 ? mark["science"] : 0;
   average+=mark["maths"]>0 ? mark["maths"] : 0;
   average+=mark["english"]>10 ? mark["english"] : 0;
   average+=mark["social"]>10 ? mark["social"] : 0;
   average=average/4;
   
   if(average<leastAverage){
    leastAverage=average;
    lastStudent=mark["first_name"]+" "+mark["last_name"];
   }
})
console.log(lastStudent,leastAverage);
}


//state which has best average
function findBestState(studentDetails){
   const states=[];
   studentDetails.forEach((student)=>{
    if(!states.includes(student["state"])){
   ...
   for(let i=0; i<states.length; i++){
    let average=0;
    let studentCount=0;
    studentDetails.forEach((studentState)=>{
        
        if(states[i]==studentState["state"]){
            average+=(studentState["science"]+studentState["maths"]+studentState["english"]+studentState["social"])/4;
            studentCount++;
        }
    })
 ...
 console.log(bestState,maxAverage);
}


//getPaginationResult
function getPaginationResult(studentDetails,page,limit){
